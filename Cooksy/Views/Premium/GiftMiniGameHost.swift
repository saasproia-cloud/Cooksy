...
/// games (`wheel → scratchCard → mysteryBox → wheel → …`) every time
/// a fresh gift cycle starts (i.e. after a reissue cooldown elapses).
/// Within a cycle the choice is stable, so closing and re-opening the
/// gift sheet always shows the same game until the user wins or the
/// cycle rolls over.
///
/// Each underlying game shares the same `(onClose, onClaim)` contract,
/// so the host just forwards the closures unchanged. To plug in a
/// fourth game, add a case to `PremiumOffersService.GiftGameKind` and
