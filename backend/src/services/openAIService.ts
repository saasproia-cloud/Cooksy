...
export async function normalizeRecipeFromContext(
  ...
  const normalizedRecipe = await requestStructuredRecipeFromOpenAI({
    ...
    systemInstructions: [
      ...
      "",
      "**RÈGLE 5 (CRITIQUE) — UNITÉ JAMAIS DANS LE NAME** —",
      "Le champ `name` contient UNIQUEMENT l'ingrédient pur (\"Sel\", \"Huile d'olive\", \"Œufs\"). L'unité va dans `unit`, la quantité dans `amount`. JAMAIS de fragment d'unité au début du `name` :",
      "  ❌ `name='À soupe huile'` → ✅ `name='Huile d'olive', unit='c. à soupe', amount='2'`",
      "  ❌ `name='À café sel'` → ✅ `name='Sel', unit='c. à café', amount='1'`",
      "  ❌ `name='300 g farine'` → ✅ `name='Farine', unit='g', amount='300'`",
      "  ❌ `name='Pincée poivre'` → ✅ `name='Poivre', unit='pincée', amount='1'`",
      "Si tu vois \"à café\", \"à soupe\", \"cuillère\", \"pincée\", \"gousse\", \"tranche\", \"sachet\", \"tasse\", \"g\", \"kg\", \"ml\", \"cl\", \"l\", \"oz\", \"tbsp\", \"tsp\", \"cup\" ou tout chiffre au DÉBUT d'un `name` que tu allais produire : ARRÊTE, déplace-le dans `unit`/`amount`, laisse uniquement l'ingrédient pur dans `name`.",
      "",
      "**RÈGLE 6 (CRITIQUE) — INGRÉDIENTS PROTÉGÉS NE DISPARAISSENT JAMAIS** —",
      "Si la source mentionne un de ces ingrédients (même brièvement, même sans quantité), tu DOIS le conserver dans `ingredientDrafts` : sel, poivre, huile (toutes huiles), beurre, sucre, eau, sauce, garniture, topping, assaisonnement, herbes, épices, \"pour servir\", \"pour la déco\", \"pour la garniture\".",
      "Ces ingrédients n'ont pas besoin d'être explicitement référencés dans une étape pour rester dans la recette. Le créateur les liste = ils restent.",
      "",
